...
#define std_sort(type)                                                  \
    BENCHMARK_TEMPLATE_F(RandomVectorFixture, BM_std_sort_##type, type) \
...
#define std_parallel_sort(type)                                             \
    BENCHMARK_TEMPLATE_F(RandomVectorFixture, BM_par_std_sort_##type, type) \
    (benchmark::State & state) {                                            \
        for (auto _ : state) {                                              \
...
#define mpqsort_sort(type)                                                            \
    BENCHMARK_TEMPLATE_F(RandomVectorFixture, BM_mpqsort_sort_##type, type)           \
    (benchmark::State & state) {                                                      \
        for (auto _ : state) {                                                        \
...
#define gnu_qs_sort(type)                                                                  \
    BENCHMARK_TEMPLATE_F(RandomVectorFixture, BM_gnu_qs_sort_##type, type)                 \
    (benchmark::State & state) {                                                           \
        for (auto _ : state) {                                                             \
...
#define gnu_bqs_sort(type)                                                  \
    BENCHMARK_TEMPLATE_F(RandomVectorFixture, BM_gnu_bqs_sort_##type, type) \
    (benchmark::State & state) {                                            \
        for (auto _ : state) {                                              \
...
#define gnu_mwms_sort(type)                                                  \
    BENCHMARK_TEMPLATE_F(RandomVectorFixture, BM_gnu_mwms_sort_##type, type) \
    (benchmark::State & state) {                                             \
        for (auto _ : state) {                                               \
            __gnu_parallel::sort(vec.begin(), vec.end(),                     \
...
#define tbb_sort(type)                                                  \
    BENCHMARK_TEMPLATE_F(RandomVectorFixture, BM_tbb_sort_##type, type) \
    (benchmark::State & state) {                                        \
        for (auto _ : state) {                                          \
...
#define nvidia_thrust_sort(type)                                                  \
    BENCHMARK_TEMPLATE_F(RandomVectorFixture, BM_nvidia_thrust_sort_##type, type) \
    (benchmark::State & state) {                                                  \
        for (auto _ : state) {                                                    \
